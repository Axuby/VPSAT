...
class WireframeDataset(Dataset):
    def __init__(self, rootdir, split):
        ...
        train_split = int(total_files * 0.7)  # 70% for training
        val_split = int(total_files * 0.85)  # 15% for validation, 15% for test

        if split == "train":
            self.filelist = filelist[:train_split]
            self.size = len(self.filelist) * C.io.augmentation_level
        elif split == "valid":
            self.filelist = filelist[train_split:val_split]
            self.size = len(self.filelist)
        elif split == "test":
            self.filelist = filelist[val_split:val_split+2*(val_split-train_split)]
            self.size = len(self.filelist)
        else:
            raise ValueError(f"Invalid split: {split}. Must be 'train', 'valid', or 'test'.")

        print(f"Number of {split} samples: {self.size}")
